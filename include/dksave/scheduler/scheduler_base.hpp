	...
	struct scheduler_base
	{
			...
			void handle_rgb(typename Camera_t::capture_loop_context & ctx, std::string const & date, std::string const & timestamp)
			{
				...
				if (handle_rgb_cost_in_milli > dksave::global_settings::get_frame_handle_timeout_rgb().count()) {
					KERBAL_LOG_WRITE(KWARNING, "Handling rgb takes time: {} ms, camera: {}, frame cnt: {}", handle_rgb_cost_in_milli, camera.device_name(), ctx.frame_count);
				} else {
					KERBAL_LOG_WRITE(KVERBOSE, "Handling rgb takes time: {} ms, camera: {}, frame cnt: {}", handle_rgb_cost_in_milli, camera.device_name(), ctx.frame_count);
				}
			...
			void handle_depth(typename Camera_t::capture_loop_context & ctx, std::string const & date, std::string const & timestamp)
			{
				...
				if (handle_depth_cost_in_milli > dksave::global_settings::get_frame_handle_timeout_depth().count()) {
					KERBAL_LOG_WRITE(KWARNING, "Handling depth takes time: {} ms, camera: {}, frame cnt: {}", handle_depth_cost_in_milli, camera.device_name(), ctx.frame_count);
				} else {
					KERBAL_LOG_WRITE(KVERBOSE, "Handling depth takes time: {} ms, camera: {}, frame cnt: {}", handle_depth_cost_in_milli, camera.device_name(), ctx.frame_count);
				}
