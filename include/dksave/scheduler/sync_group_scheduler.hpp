			...
			void group_stabilize() try
			{
				for (std::size_t i = 0; i < this->ctx_group.size(); ++i) {
					...
					try {
						...
						KERBAL_LOG_WRITE(KERROR, "Stabilization process failed. camera: {}",
										 camera.device_name());
						throw;
					}
				}
			...
			void thread()
			{
				while (true) {
					...
					if (status != group_previous_status) {
						KERBAL_LOG_WRITE(KINFO, "Camera status switched. master camera: {}, from: {}, to: {}",
					...
					if (status == dksave::working_status::SLEEP) {
						...
						if (!master_camera.enable()) {
							// 启动设备
							KERBAL_LOG_WRITE(KINFO, "Group is sleeping, try to wake up... . master_camera: {}", master_camera.device_name());
							try {
								group_start();
								KERBAL_LOG_WRITE(KINFO, "Group has been started. master_camera: {}", master_camera.device_name());
							} catch (...) {
								KERBAL_LOG_WRITE(KERROR, "Group starts failed. master_camera: {}, next try period: 30s", master_camera.device_name());
								std::this_thread::sleep_for(30s);
								continue;
							}
							try {
								KERBAL_LOG_WRITE(KINFO, "Stabilizing the group. master_camera: {}", master_camera.device_name());
								group_stabilize();
								KERBAL_LOG_WRITE(KINFO, "Group has been stabilized. master_camera: {}", master_camera.device_name());
							} catch (...) {
								KERBAL_LOG_WRITE(KERROR, "Stabilizing the group failed. master_camera: {}, next try period: 30s",
			...
			void one_capture()
			{
				...
				for (std::size_t i = 0; i < this->ctx_group.size(); ++i) {
					...
					try {
						...
						KERBAL_LOG_WRITE(KINFO, "Capture handled success. camera: {}, frame_count: {}",
										 camera.device_name(),
										 ctx.frame_count);
