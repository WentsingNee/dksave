		...
		public:
			...
			)
			{
				...
				try {
					...
					KERBAL_LOG_WRITE(KERROR, "Depth image transformation to color failed. camera: {}, exception_type: {}, what: {}",
									 camera->device_name(),
									 typeid(e).name(),
									 e.what()
					);
					...
					KERBAL_LOG_WRITE(KERROR, "Depth image transformation to color failed. camera: {}, exception_type: unknown",
									 camera->device_name()
					);
					return;
				}
