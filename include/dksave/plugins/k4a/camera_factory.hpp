			...
			)
			{
				...
				for (auto const & sync_group : sync_groups_node) {
					...
					if (not subordinates_node.IsSequence()) {
						KERBAL_LOG_WRITE(
							KFATAL, "subordinates should be list. master: {}",
							master_snid
						);
						exit(EXIT_FAILURE);
					}
					if (auto it = sub_2_master.find(master_snid); it != sub_2_master.cend()) {
						KERBAL_LOG_WRITE(
							KFATAL, "{} can't be a master because it has been designated as {}'s subordinate",
					...
					for (auto const & subordinate_entry : subordinates_node) {
						...
						if (master_2_sub.contains(subordinate_snid)) {
							KERBAL_LOG_WRITE(
								KFATAL, "{} can't be designated as {}'s subordinate because it's a master",
								subordinate_snid, master_snid
							);
							exit(EXIT_FAILURE);
						}
						if (auto it = sub_2_master.find(subordinate_snid); it != sub_2_master.cend()) {
							KERBAL_LOG_WRITE(
								KFATAL, "{} can't be designated as {}'s subordinate because it has been designated as {}'s subordinate before",
								subordinate_snid, master_snid, it->value()
							);
							exit(EXIT_FAILURE);
						}
						...
						KERBAL_LOG_WRITE(
							KINFO, "Add sync relationship. master: {}, subordinate: {}",
							master_snid, subordinate_snid
						);
					}
				}
			...
			find_cameras(YAML::Node const & yaml_config)
			{
				...
				for (auto & camera_ : cameras) {
					...
					switch (camera_.config().wired_sync_mode) {
						case K4A_WIRED_SYNC_MODE_STANDALONE: {
							...
							KERBAL_LOG_WRITE(
								KINFO, "Grouped a standalone camera. device name: {}, group: {}",
								device_name, cameras_by_group.size()
							);
							break;
						}
						case K4A_WIRED_SYNC_MODE_MASTER: {
							...
							KERBAL_LOG_WRITE(
								KINFO, "Grouped a master camera. device name: {}, group: {}",
								device_name, cameras_by_group.size()
							);
							break;
						}
				...
				for (auto & sub_camera : cameras) {
					...
					if (it != cameras_by_group.end()) {
						...
						KERBAL_LOG_WRITE(
							KINFO, "Grouped a subordinate camera. device name: {}, group: {}",
							device_name, it - cameras_by_group.begin()
						);
					}
