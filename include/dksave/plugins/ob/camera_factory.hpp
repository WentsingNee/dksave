			...
			void open_local_cameras()
			{
				...
				for (uint32_t i = 0; i < device_count; ++i) {
					...
					try {
						...
						KERBAL_LOG_WRITE(KERROR, "Opening local camera failed. index: {}, exception type: {}, what: {}",
										 i, typeid(e).name(), e.what());
					} catch (...) {
						KERBAL_LOG_WRITE(KERROR, "Opening local camera failed. index: {}, exception type: unknown", i);
					}
				}
