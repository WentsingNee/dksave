...
class H264_decode_error : std::runtime_error
{
	...
	public:
		H264_decode_error(std::string const & msg) : super(msg)
		{
		}
};
...
struct av_allocator<void>
{
	...
	static value_type * allocate(size_type size) {
	...
	static void deallocate(value_type * p, size_type) noexcept {
		av_free(p);
	}
};
...
struct av_allocator
{
	...
	public:
		...
		static value_type * allocate(size_type n) {
			return static_cast<value_type*>(upstream_allocator::allocate(n * sizeof(value_type)));
		}
...
cv::Mat const & H264_to_cv_mat_context::decode(std::uint8_t *input_buff, size_t size) {
	if (0 == size) {
...
//	this->av_packet->size = this->in_buffer.size();
	...
	std::unique_ptr<SwsContext, SwsContextDeleter> sws_context;
	{
		SwsContext * p_sws_context =
				sws_getContext(av_codec_context->width, av_codec_context->height, av_codec_context->pix_fmt,
	...
	int BGR_size = av_image_get_buffer_size(AV_PIX_FMT_BGR24, av_codec_context->width, av_codec_context->height, 1);
	out_buffer.resize(BGR_size);
	if (0 > av_image_fill_arrays(av_frame_bgr->data, av_frame_bgr->linesize, &out_buffer[0], AV_PIX_FMT_BGR24,
						 av_codec_context->width, av_codec_context->height, 1)) {
		throw H264_decode_error("av_image_fill_arrays failed");
	}

	sws_scale(sws_context.get(), (const uint8_t *const *) av_frame->data,
			  av_frame->linesize, 0, av_codec_context->height, av_frame_bgr->data, av_frame_bgr->linesize);

	this->cv_mat = cv::Mat(cv::Size(av_codec_context->width, av_codec_context->height), CV_8UC3, &out_buffer[0]);
