...
export module dksave.plugins.ob.camera;

import dksave.plugins.ob.config;
import dksave.plugins.ob.context.H264_to_cv_mat_context_t;
import dksave.plugins.ob.context.ob_color_frame_to_cv_mat_context_t;
import dksave.plugins.ob.context.ob_frame_depth_to_cv_mat_context_t;
import dksave.plugins.ob.context.ob_img_depth_transform_to_color_mode_context_t;

import dksave.context.image_rotate_context_t;
import dksave.plugins.ucamera;

import dksave.logger;
import dksave.rotate_flag_t;
import dksave.registration_mode_t;
import dksave.save_cv_mat;
			...
			)
			{
				...
				if (camera->registration_mode() == registration_mode_t::DEPTH_TO_COLOR) {
					try {
						...
						KERBAL_LOG_WRITE(
							KERROR, "Transforming depth image to color type failed. camera: {}, exception type: {}, what: {}",
							camera->device_name(),
							typeid(e).name(), e.what()
						);
						throw;
						...
						KERBAL_LOG_WRITE(
							KERROR, "Transforming depth image to color type failed. camera: {}, exception type: unknown",
				...
				try {
					...
					KERBAL_LOG_WRITE(
						KERROR, "Depth image saved failed. camera: {}, filename_depth: {}, exception type: {}, what: {}",
						camera->device_name(),
						filename_depth.string(),
						typeid(e).name(), e.what()
					);
